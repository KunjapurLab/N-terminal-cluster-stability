...
class MyWindow(QWidget):
    ...
    def init_ui(self):
        ...
        self.text_inputs = [
            ...
            {'label': 'What is the amino acid you are interested in tracking?', 'input': QLineEdit()},
    ...
    def submit_inputs(self):
        ...
        [NGS_file, output_file, common, num_degen, AA_target, AA_position] = user_inputs()
        ...
        target_count = position_counting(count_full_AA, AA_target, AA_position, num_degen)
        [Positions, AA_count] = before_after(AA_target, AA_position, num_degen, count_full_AA)
        write_output(output_file, AA_target, AA_position, common_count, target_count, num_reads, num_degen, AA_count, Positions)
        print("Output Generated")
        
    
def user_inputs():
    FASTQ_file = window.file_input_values[0]
    output_file = str(window.text_input_values[4])+'.txt'
    ...
    return [FASTQ_file, output_file, common, num_degen,AA_target,AA_position]
...
def position_counting (AA_count, AA_target, AA_position, num_degen):
    target_counter = 0
    Pos2 = []
    if AA_position > num_degen:
        return []
    else:
        for key in AA_count:
            if key[int(AA_position)-1] == str(AA_target):
                target_counter += int(AA_count[key])
    return target_counter            


def before_after (AA_target, AA_position, num_degen, deg_library):
    target_counter = 0
    ...
    for key in deg_library:
        if key[int(AA_position)-1] == str(AA_target):
            target_counter += int(deg_library[key])
            for i in range(len(key)):
                for AA in AA_match:
                    if i<int(AA_position)-1 and key[i] == AA:
                        AA_counts[i][AA] += deg_library[key]
                    if i>int(AA_position)-1 and key[i] == AA:
                        AA_counts[i-1][AA] += deg_library[key]
        
    Positions = ['P'+str(i+1) for i in range(int(num_degen)+1) if i != int(AA_position)-1]
    return [Positions, AA_counts]

def write_output(output_file, AA_target, AA_position, common_count, target_count, num_reads, num_degen, AA_count, Positions):
    '''
